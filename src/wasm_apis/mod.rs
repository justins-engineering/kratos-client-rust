...
pub(crate) async fn handle_response<T, E>(resp: web_sys::Response) -> Result<T, Error<E>>
where
    ...
    E: serde::de::DeserializeOwned,
{
    ...
    let content = wasm_bindgen_futures::JsFuture::from(resp.json()?).await?;
    ...
    if !(400..600).contains(&status) {
        ...
        let entity: Option<E> = content.into_serde().ok();
...
pub(crate) async fn handle_empty_response<E>(resp: web_sys::Response) -> Result<(), Error<E>>
where
    E: serde::de::DeserializeOwned,
{
    let status = resp.status();
